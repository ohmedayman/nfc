  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === "auth/popup-closed-by-user") {
        ...
        setError("تم حظر النافذة المنبثقة. سمح للمتصفح بها");
      } else {
      ...
      <div className="w-full max-w-[450px]">
        ...
        <div className="bg-white rounded-3xl shadow-[0_1px_3px_rgba(0,0,0,0.12)] px-10 pt-10 pb-8">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-4 py-4 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <div className="w-5 h-5 border-2 border-gray-300 border-t-[#1a73e8] rounded-full animate-spin" />
            ) : (
              <svg className="w-6 h-6" viewBox="0 0 48 48">
                <path
                  fill="#EA4335"
